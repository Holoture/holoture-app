/**
 * Shared logic for the 8x/day scheduled-signal generator — premarket,
 * regular-hours, and after-hours slots.
 *
 * Extracted out of app/api/cron/scheduled-signals/route.ts so each slot can
 * live at its OWN route pathname (app/api/cron/scheduled-signals/<slot>/route.ts)
 * instead of all 8 sharing one pathname distinguished only by a `?slot=`
 * query string. Diagnosis (see the Vercel Cron Jobs "Runs" log, which showed
 * zero real invocations ever for the shared-pathname route despite it being
 * correctly registered) points at Vercel's cron scheduler not reliably
 ...
 * across the day, per SLOTS below.
 *
 * Claude never decides inclusion (same contract as cron/momentum) — every
 * candidate has already passed hard numeric gates before the model sees it,
 * and only writes company name + thesis. Entry/target/stop/confidence/
 * signalType are all computed mechanically, so confidence can never come
 * back null.
 */
...
/**
 * Runs one slot end-to-end and returns the JSON-serializable result.
 * isDryRun=true never calls Claude or writes — it reports what WOULD be
 * created (with the full rejection breakdown) so it can be safely re-run
 * for diagnosis without persisting anything or spending model tokens.
 */
export async function runSlot(slotId: SlotId, isDryRun: boolean) {
  const slot = SLOTS[slotId]

  // Universe = the weekly-screened pool, exactly like cron/signals — this IS
  // the daily-average dollar-volume liquidity floor, identical for every
  // slot including premarket/after-hours.
  const universeRows = await prisma.tickerUniverse.findMany({
    select: { ticker: true, marketCapBand: true },
  })
  if (universeRows.length === 0) {
    return { ok: true, slot: slotId, scanned: 0, note: 'TickerUniverse empty — run cron/universe-screen' }
  }
