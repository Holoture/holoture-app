...
const signals = [
  {
    ...
    thesis:
      'Blackwell GPU shipments accelerating into H2 with sovereign AI demand from Middle East and Asia offsetting any near-term hyperscaler capex caution. Data center revenue run-rate supports a 28–30x forward earnings multiple at current entry.',
    aiSummary:
  ...
  },
  {
    ...
    timeHorizon: '3–6 months',
    thesis:
      'Net interest income holding firm despite rate-cut cycle with loan growth re-accelerating. Investment banking pipeline at multi-year high. Jamie Dimon buyback authorization signals management confidence in capital position.',
    aiSummary:
  ...
  },
  {
    ...
    timeHorizon: '6–12 months',
    thesis:
      'International upstream capex still growing at high single digits despite softness in North American land. Middle East NOC spend is structural and insulated from spot crude moves. Valuation at 12x forward earnings is near trough for a business with recurring digital/software revenue.',
    aiSummary:
      'Best-positioned oilfield services company for international capex cycle. Digital segment growing 20%+ YoY adds valuation multiple support. Risk: WTI below $65 forces NOC budget cuts.',
  },
  {
    ticker: 'PLTR',
    ...
    thesis:
      'Trading at 90x forward revenue — priced for perfection in an environment where DoD budget uncertainty and continuing resolutions threaten government contract timing. Commercial revenue growth decelerating sequentially. Risk/reward skews short on any guidance miss.',
    aiSummary:
      'Stretched valuation leaves no margin for error. Government segment exposed to defense budget volatility; commercial growth rate is plateauing. Short with tight stop above $38 resistance.',
  },
]
...
async function main() {
  ...
  for (const signal of signals) {
    ...
    console.log(`  ✓ ${signal.ticker} — ${signal.signalType}`)
    created++
  }
  console.log(`\nDone. Created ${created} signals.`)
}
