...
type GeneratedSignal = {
  ...
  confidence: number
  timeHorizon: string
  // Structured enum, assigned by Claude directly and validated server-side —
  // timeHorizon stays a free-text DISPLAY string, never parsed for
  // classification again. 'momentum' is never legal here; validateSignal()
  // rejects it (reserved for the real intraday scanner).
  timeframeCategory: string
  thesis: string
  aiSummary: string
  sector: string
  // Event-driven vetted-signal tag — see prisma/schema.prisma's
  // Signal.catalystType comment. Claude sets this (or leaves it null)
  // during the SAME generation pass as everything else above; there is no
  // separate detection pipeline. Validated server-side in validateSignal()
  // before ever reaching prisma.signal.create().
...
- Include a mix of timeframes — pick a human-readable display phrase for timeHorizon AND its matching timeframeCategory enum value from this exact mapping:
  - "1-3 days" (short-term catalyst/technical setup) -> timeframeCategory "days_1_3"
  - "2-4 weeks" (swing) -> timeframeCategory "swing"
  - "1-3 months" -> timeframeCategory "swing" (NOT "momentum" — that category is reserved for a separate real-time intraday scanner and must never be used here)
  - "3-6 months" (long-term) -> timeframeCategory "long_term"
  - "6-12 months" or longer -> timeframeCategory "long_term"
...
- For EVERY signal, also decide whether it is genuinely event-driven: is the PRIMARY reason for this setup an identifiable, concrete event — an earnings report/beat/miss, a contract or purchase-order win, an M&A announcement, an FDA decision, or a guidance change — rather than a pure technical/momentum pattern? Only set catalystType when the data you were given actually supports it (recentHeadlines, fundamentals showing a clear earnings-driven move, etc.) — never invent a catalyst that isn't backed by what's in the stock data below. Most signals are NOT event-driven and should have catalystType: null. This is independent of the thesis's own CATALYST segment — catalystType is a structured tag, catalystSummary is a short plain-language restatement of the same real event (e.g. "Q2 earnings beat: gross margin expanded 530bps"), not free-form commentary.
...
- timeHorizon (string — the human-readable display phrase, e.g. "1-3 months")
- timeframeCategory (string — EXACTLY one of: "days_1_3", "swing", "long_term". Never "intraday" and never "momentum" — those are assigned by other, separate crons.)
...
- catalystType (string or null — EXACTLY one of "EARNINGS", "CONTRACT", "MA", "FDA", "GUIDANCE", "OTHER" if this setup is genuinely event-driven and the data supports it, otherwise null. Most signals should be null.)
- catalystSummary (string or null — REQUIRED if catalystType is non-null, otherwise null. One short plain-language sentence naming the actual event, e.g. "Q2 earnings beat: gross margin expanded 530bps". Never populate this without a matching catalystType.)
...
// This cron may only ever assign days_1_3 / swing / long_term — 'intraday'
// and 'momentum' belong exclusively to other crons. Falls back to the
// legacy free-text classifier (never returns 'momentum') if Claude's value
// is missing, malformed, or one of the two categories this cron may not use.
const ALLOWED_HERE: ReadonlySet<TimeframeCategory> = new Set(['days_1_3', 'swing', 'long_term'])
function resolveTimeframeCategory(s: GeneratedSignal): TimeframeCategory {
  if (isValidTimeframeCategory(s.timeframeCategory) && ALLOWED_HERE.has(s.timeframeCategory)) {
    return s.timeframeCategory
  }
  return classifyLegacyTimeHorizon(s.timeHorizon) ?? 'swing'
}

// Never trust Claude's catalystType/catalystSummary blindly — an invalid
// enum value, or a type with no matching summary (or vice versa), falls
// back to null/null rather than writing a half-populated or garbage tag.
// Most signals correctly resolve to { type: null, summary: null }.
function resolveCatalystTag(s: GeneratedSignal): { type: string | null; summary: string | null } {
  const type = s.catalystType ?? null
  const summary = s.catalystSummary?.trim() || null
  if (!isValidCatalystType(type) || !summary) return { type: null, summary: null }
  return { type, summary }
}
...
export async function GET(req: Request) {
  ...
  try {
    // Was a 24h-freshness window (autoGenerated only) — a still-active
    // signal older than 24h (now common: swing/long_term stay active up to
    // 90 days) was invisible here and could get a duplicate, while the
    // separate 48h staleness sweep below hadn't retired it yet either. That
    // 24-48h gap produced real, live duplicates (confirmed during the
    // duplicate-active-signals audit — e.g. a pair generated ~36.6h apart).
    // Fixed: true active-signal check, this cron's own session ('regular',
    // always — never set explicitly below), no time window. Includes manual
    // signals too (no autoGenerated filter) — a hand-entered signal blocks a
    // new algo one on the same ticker+session, per that audit's decision.
    ...
    const filteredLargeCap = largeUniverse.filter((t) => !activeTickers.has(t))
    const filteredSmallCap = smallUniverse.filter((t) => !activeTickers.has(t))
    ...
    const seenTickers = new Set<string>(activeTickers)
    ...
    await prisma.$transaction([
      ...
      ...allSignals.map((s) => {
        const catalystTag = resolveCatalystTag(s)
        return prisma.signal.create({
          data: {
            ...
            catalystType: catalystTag.type,
            catalystSummary: catalystTag.summary,
            isActive: true,
            autoGenerated: true,
          },
        })
