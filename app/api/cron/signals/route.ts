...
export async function GET(req: Request) {
  ...
  try {
    // ── Retention: find tickers that already have an active signal from the last 24h ──
    ...
    // Only process tickers that don't already have a fresh signal
    ...
    // If every ticker already has a fresh signal, skip generation entirely
    ...
    // Fetch quotes for both universes in parallel (only uncovered tickers)
    ...
    // Validate and deduplicate — pre-seed with fresh tickers so AI can't duplicate them
    ...
    // Deactivate signals older than 48h — never delete, only flip isActive
