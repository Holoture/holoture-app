/**
 * GET /api/cron/scheduled-signals/regular-1130
 *
 * Dedicated pathname for the 11:30am regular-hours slot — see
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const result = await runSlot('regular_1130', isDryRun)
    return NextResponse.json(result)
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err)
    console.error('[cron/scheduled-signals/regular-1130]', msg)
