/**
 * GET /api/cron/scheduled-signals/regular-1515
 *
 * Dedicated pathname for the 3:15pm regular-hours slot — see
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const result = await runSlot('regular_1515', isDryRun)
    return NextResponse.json(result)
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err)
    console.error('[cron/scheduled-signals/regular-1515]', msg)
