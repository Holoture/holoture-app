/**
 * GET /api/cron/scheduled-signals/regular-1400
 *
 * Dedicated pathname for the 2:00pm regular-hours slot — see
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const result = await runSlot('regular_1400', isDryRun)
    return NextResponse.json(result)
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err)
    console.error('[cron/scheduled-signals/regular-1400]', msg)
