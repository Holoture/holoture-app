 ...
 *   - /movers, /tracker, /admin/*, /account/*, /sign-in, /sign-up: hard
 *     server-side redirect('/sign-in') for a signed-out visitor, confirmed
 *     via live anonymous request (307, Location header present). Already
 *     disallowed in robots.txt for the admin/account/auth paths too.
 ...
 *     That's an inconsistency between these three routes and movers/
 *     tracker/admin under the identical redirect() call — flagged as a bug
 *     to investigate separately. Excluded here based on the clear code
 *     intent and real-browser behavior, not the anomalous raw-HTTP 200.
 *
